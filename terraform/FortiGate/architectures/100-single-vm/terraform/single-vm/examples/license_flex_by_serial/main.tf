...
resource "fortiflexvm_entitlements_vm_token" "fgt" {
  config_id = local.serials_to_config[var.serial_number]
  serial_number = var.serial_number
  regenerate_token = false # If set as false, the provider will only provide the token and not regenerate it.
}
...
module "fgt" {
    ...
    flex_token = fortiflexvm_entitlements_vm_token.fgt.token
}
