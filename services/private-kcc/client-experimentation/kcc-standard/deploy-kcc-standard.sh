...
# add 172.16.0.32/28 - GKE cluster
...
# Grant GCP role Network Admin to the yakima service account
# This is required to deploy the GKE Cluster network and subnet
gcloud projects add-iam-policy-binding "${PROJECT_ID}" \
  --member "serviceAccount:${SA_EMAIL}" \
  --role "roles/compute.networkAdmin" \
  --project "${PROJECT_ID}"

# Grant GCP role Compute Security Admin to the yakima service account
# This is required to create firewalls rules for GKE
gcloud projects add-iam-policy-binding "${PROJECT_ID}" \
  --member "serviceAccount:${SA_EMAIL}" \
